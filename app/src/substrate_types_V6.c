...
parser_error_t _readOptionElectionScore_V6(parser_context_t* c, pd_OptionElectionScore_V6_t* v)
{
    CHECK_ERROR(_readUInt8(c, &v->some))
    if (v->some > 0) {
        CHECK_ERROR(_readElectionScore_V6(c, &v->contained))
    }
    return parser_ok;
}
...
parser_error_t _readOptionMultiSignature_V6(parser_context_t* c, pd_OptionMultiSignature_V6_t* v)
{
    CHECK_ERROR(_readUInt8(c, &v->some))
    if (v->some > 0) {
        CHECK_ERROR(_readMultiSignature_V6(c, &v->contained))
    }
    return parser_ok;
}
...
parser_error_t _readOptionMultiSigner_V6(parser_context_t* c, pd_OptionMultiSigner_V6_t* v)
{
    CHECK_ERROR(_readUInt8(c, &v->some))
    if (v->some > 0) {
        CHECK_ERROR(_readMultiSigner_V6(c, &v->contained))
    }
    return parser_ok;
}
...
parser_error_t _readOptionPercent_V6(parser_context_t* c, pd_OptionPercent_V6_t* v)
{
    CHECK_ERROR(_readUInt8(c, &v->some))
    if (v->some > 0) {
        CHECK_ERROR(_readPercent_V6(c, &v->contained))
    }
    return parser_ok;
}
    ...
    uint8_t* pageCount)
{
    ...
    *pageCount = 1;
    if (v->some > 0) {
        CHECK_ERROR(_toStringElectionScore_V6(
            &v->contained,
            outValue, outValueLen,
            pageIdx, pageCount));
    } else {
    ...
    uint8_t* pageCount)
{
    ...
    *pageCount = 1;
    if (v->some > 0) {
        CHECK_ERROR(_toStringMultiSignature_V6(
            &v->contained,
            outValue, outValueLen,
            pageIdx, pageCount));
    } else {
    ...
    uint8_t* pageCount)
{
    ...
    *pageCount = 1;
    if (v->some > 0) {
        CHECK_ERROR(_toStringMultiSigner_V6(
            &v->contained,
            outValue, outValueLen,
            pageIdx, pageCount));
    } else {
    ...
    uint8_t* pageCount)
{
    ...
    *pageCount = 1;
    if (v->some > 0) {
        CHECK_ERROR(_toStringPercent_V6(
            &v->contained,
            outValue, outValueLen,
            pageIdx, pageCount));
    } else {
