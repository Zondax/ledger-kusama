...
parser_error_t _readCompactPropIndex(parser_context_t *c,
                                     pd_CompactPropIndex_t *v) {
  return _readCompactInt(c, v);
}

parser_error_t _readCompactReferendumIndex(parser_context_t *c,
                                           pd_CompactReferendumIndex_t *v) {
  return _readCompactInt(c, v);
}

parser_error_t _readCompactMemberCount(parser_context_t *c,
                                       pd_CompactMemberCount_t *v) {
  return _readCompactInt(c, v);
}
...
parser_error_t _readCompactBountyIndex(parser_context_t *c,
                                       pd_CompactBountyIndex_t *v) {
  return _readCompactInt(c, v);
}

parser_error_t _readCompactRegistrarIndex(parser_context_t *c,
                                          pd_CompactRegistrarIndex_t *v) {
  return _readCompactInt(c, v);
}
...
parser_error_t _readAccountVoteStandard(parser_context_t *c,
                                        pd_AccountVoteStandard_t *v) {
  CHECK_ERROR(_readVote(c, &v->vote));
  CHECK_ERROR(_readBalanceOf(c, &v->balance));
  return parser_ok;
}

parser_error_t _readAccountVoteSplit(parser_context_t *c,
                                     pd_AccountVoteSplit_t *v) {
...
parser_error_t
    _readValidatorPrefs(parser_context_t *c, pd_ValidatorPrefs_t *v) {
  CHECK_INPUT();
...
parser_error_t _readTupleBalanceOfBalanceOfBlockNumber(
    parser_context_t *c, pd_TupleBalanceOfBalanceOfBlockNumber_t *v) {
...
parser_error_t
    _readReferendumIndex(parser_context_t *c, pd_ReferendumIndex_t *v) {
  return _readUInt32(c, &v->value);
}
...
parser_error_t
    _readEthereumAddress(parser_context_t *c,
                         pd_EthereumAddress_t *v){GEN_DEF_READARRAY(20)}
...
parser_error_t _readRegistrarIndex(parser_context_t *c,
                                   pd_RegistrarIndex_t *v) {
  return _readUInt32(c, &v->value);
}

parser_error_t _readIdentityFields(parser_context_t *c,
                                   pd_IdentityFields_t *v) {
  return parser_not_supported;
}
...
parser_error_t _readu8_array_32(parser_context_t *c,
                                pd_u8_array_32_t *v){GEN_DEF_READARRAY(32)}

parser_error_t
    _readVecTupleDataData(parser_context_t *c, pd_VecTupleDataData_t *v){
...
parser_error_t _readVecTupleAccountIdData(parser_context_t *c,
                                          pd_VecTupleAccountIdData_t *v){
    GEN_DEF_READVECTOR(TupleAccountIdData)}

parser_error_t
    _readOptionu8_array_20(parser_context_t *c, pd_Optionu8_array_20_t *v) {
...
parser_error_t _readOptionTupleBalanceOfBalanceOfBlockNumber(
    parser_context_t *c, pd_OptionTupleBalanceOfBalanceOfBlockNumber_t *v) {
  CHECK_ERROR(_readUInt8(c, &v->some))
  if (v->some > 0) {
...
parser_error_t _readOptionReferendumIndex(parser_context_t *c,
                                          pd_OptionReferendumIndex_t *v) {
  CHECK_ERROR(_readUInt8(c, &v->some))
  if (v->some > 0) {
...
parser_error_t _readOptionAccountId(parser_context_t *c,
                                    pd_OptionAccountId_t *v) {
  CHECK_ERROR(_readUInt8(c, &v->some))
  if (v->some > 0) {
...
parser_error_t _readOptionStatementKind(parser_context_t *c,
                                        pd_OptionStatementKind_t *v) {
  CHECK_ERROR(_readUInt8(c, &v->some))
  if (v->some > 0) {
...
parser_error_t _readOptionProxyType(parser_context_t *c,
                                    pd_OptionProxyType_t *v) {
  CHECK_ERROR(_readUInt8(c, &v->some))
  if (v->some > 0) {
...
parser_error_t _readOptionTimepoint(parser_context_t *c,
                                    pd_OptionTimepoint_t *v) {
  CHECK_ERROR(_readUInt8(c, &v->some))
  if (v->some > 0) {
...
parser_error_t _toStringCompactPropIndex(const pd_CompactPropIndex_t *v,
                                         char *outValue, uint16_t outValueLen,
                                         uint8_t pageIdx, uint8_t *pageCount) {
  return _toStringCompactInt(v, 0, 0, outValue, outValueLen, pageIdx,
                             pageCount);
...
parser_error_t
_toStringCompactReferendumIndex(const pd_CompactReferendumIndex_t *v,
                                char *outValue, uint16_t outValueLen,
                                uint8_t pageIdx, uint8_t *pageCount) {
  return _toStringCompactInt(v, 0, 0, outValue, outValueLen, pageIdx,
                             pageCount);
...
parser_error_t _toStringCompactMemberCount(const pd_CompactMemberCount_t *v,
                                           char *outValue, uint16_t outValueLen,
                                           uint8_t pageIdx,
                                           uint8_t *pageCount) {
  return _toStringCompactInt(v, 0, 0, outValue, outValueLen, pageIdx,
...
parser_error_t _toStringCompactWeight(const pd_CompactWeight_t *v,
                                      char *outValue, uint16_t outValueLen,
                                      uint8_t pageIdx, uint8_t *pageCount) {
  return _toStringCompactInt(v, 0, 0, outValue, outValueLen, pageIdx,
                             pageCount);
...
parser_error_t _toStringCompactBountyIndex(const pd_CompactBountyIndex_t *v,
                                           char *outValue, uint16_t outValueLen,
                                           uint8_t pageIdx,
                                           uint8_t *pageCount) {
  return _toStringCompactInt(v, 0, 0, outValue, outValueLen, pageIdx,
...
parser_error_t
_toStringCompactRegistrarIndex(const pd_CompactRegistrarIndex_t *v,
                               char *outValue, uint16_t outValueLen,
                               uint8_t pageIdx, uint8_t *pageCount) {
  return _toStringCompactInt(v, 0, 0, outValue, outValueLen, pageIdx,
                             pageCount);
...
parser_error_t _toStringConviction(const pd_Conviction_t *v, char *outValue,
...
parser_error_t _toStringVote(const pd_Vote_t *v, char *outValue,
                             uint16_t outValueLen, uint8_t pageIdx,
                             uint8_t *pageCount) {
  CLEAN_AND_CHECK()
  ...
  outValue[offset++] = ' ';
  CHECK_PARSER_ERR(_toStringConviction(&v->conviction, outValue + offset,
                                       outValueLen - offset, pageIdx,
                                       pageCount));
  *pageCount = 1;
  return parser_ok;
}
...
parser_error_t _toStringAccountVoteStandard(const pd_AccountVoteStandard_t *v,
                                            char *outValue,
                                            uint16_t outValueLen,
                                            uint8_t pageIdx,
                                            uint8_t *pageCount) {
  ...
  CHECK_ERROR(
      _toStringBalanceOf(&v->balance, outValue, outValueLen, 0, &pages[2]));
  ...
  *pageCount = 0;
  ...
  if (pageIdx < pages[1]) {
    CHECK_ERROR(
        _toStringVote(&v->vote, outValue, outValueLen, pageIdx, &pages[1]));
    return parser_ok;
  }
  pageIdx -= pages[1];
  ...
  if (pageIdx < pages[2]) {
    CHECK_ERROR(_toStringBalanceOf(&v->balance, outValue, outValueLen, pageIdx,
                                   &pages[2]));
    return parser_ok;
  }
  pageIdx -= pages[2];
...
parser_error_t _toStringAccountVoteSplit(const pd_AccountVoteSplit_t *v,
                                         char *outValue, uint16_t outValueLen,
                                         uint8_t pageIdx, uint8_t *pageCount) {
  CLEAN_AND_CHECK()
  // First measure number of pages
  ...
  *pageCount = 0;
  ...
  if (pageIdx < pages[1]) {
    CHECK_ERROR(
        _toStringBalanceOf(&v->aye, outValue, outValueLen, pageIdx, &pages[1]));
    return parser_ok;
  }
  pageIdx -= pages[1];
  ...
  if (pageIdx < pages[2]) {
    CHECK_ERROR(
        _toStringBalanceOf(&v->nay, outValue, outValueLen, pageIdx, &pages[2]));
    return parser_ok;
  }
  pageIdx -= pages[2];
...
parser_error_t _toStringAccountVote(const pd_AccountVote_t *v, char *outValue,
                                    uint16_t outValueLen, uint8_t pageIdx,
                                    uint8_t *pageCount) {
  CLEAN_AND_CHECK()
  switch (v->value) {
  case 0:
    _toStringAccountVoteStandard(&v->voteStandard, outValue, outValueLen,
                                 pageIdx, pageCount);
    break;
  case 1:
    _toStringAccountVoteSplit(&v->voteSplit, outValue, outValueLen, pageIdx,
                              pageCount);
    break;
  default:
    return parser_unexpected_value;
...
parser_error_t _toStringTupleBalanceOfBalanceOfBlockNumber(
    const pd_TupleBalanceOfBalanceOfBlockNumber_t *v, char *outValue,
  ...
  uint8_t pages[3];
  CHECK_ERROR(
      _toStringBalanceOf(&v->balance1, outValue, outValueLen, 0, &pages[0]))
  CHECK_ERROR(
      _toStringBalanceOf(&v->balance2, outValue, outValueLen, 0, &pages[1]))
  CHECK_ERROR(_toStringBlockNumber(&v->blockNumber, outValue, outValueLen, 0,
                                   &pages[2]))
  ...
  *pageCount = pages[0] + pages[1] + pages[2];
  ...
  if (pageIdx < pages[0]) {
    CHECK_ERROR(_toStringBalanceOf(&v->balance1, outValue, outValueLen, pageIdx,
                                   &pages[0]))
    return parser_ok;
  }
  pageIdx -= pages[0];
  ...
  //////
  if (pageIdx < pages[1]) {
    CHECK_ERROR(_toStringBalanceOf(&v->balance2, outValue, outValueLen, pageIdx,
                                   &pages[1]))
    return parser_ok;
  }
  pageIdx -= pages[1];
  ...
  //////
  if (pageIdx < pages[2]) {
    CHECK_ERROR(_toStringBlockNumber(&v->blockNumber, outValue, outValueLen,
                                     pageIdx, &pages[2]))
    return parser_ok;
  }
...
parser_error_t _toStringIdentityInfo(const pd_IdentityInfo_t *v, char *outValue,
                                     uint16_t outValueLen, uint8_t pageIdx,
                                     uint8_t *pageCount) {
  CLEAN_AND_CHECK()
...
parser_error_t _toStringVestingInfo(const pd_VestingInfo_t *v, char *outValue,
                                    uint16_t outValueLen, uint8_t pageIdx,
                                    uint8_t *pageCount) {
  CLEAN_AND_CHECK()
  ...
  uint8_t pages[3];
  CHECK_ERROR(
      _toStringBalanceOf(&v->locked, outValue, outValueLen, 0, &pages[0]))
  CHECK_ERROR(
      _toStringBalanceOf(&v->per_block, outValue, outValueLen, 0, &pages[1]))
  ...
  *pageCount = pages[0] + pages[1] + pages[2];
  ...
  if (pageIdx < pages[0]) {
    CHECK_ERROR(_toStringBalanceOf(&v->locked, outValue, outValueLen, pageIdx,
                                   &pages[0]))
    return parser_ok;
  }
  pageIdx -= pages[0];
  ...
  //////
  if (pageIdx < pages[1]) {
    CHECK_ERROR(_toStringBalanceOf(&v->per_block, outValue, outValueLen,
                                   pageIdx, &pages[1]))
    return parser_ok;
  }
  pageIdx -= pages[1];
...
parser_error_t _toStringReferendumIndex(const pd_ReferendumIndex_t *v,
                                        char *outValue, uint16_t outValueLen,
                                        uint8_t pageIdx, uint8_t *pageCount) {
  return _toStringu32(&v->value, outValue, outValueLen, pageIdx, pageCount);
}
...
parser_error_t _toStringDefunctVoter(const pd_DefunctVoter_t *v, char *outValue,
                                     uint16_t outValueLen, uint8_t pageIdx,
                                     uint8_t *pageCount) {
  CLEAN_AND_CHECK()
...
parser_error_t _toStringRenouncing(const pd_Renouncing_t *v, char *outValue,
                                   uint16_t outValueLen, uint8_t pageIdx,
                                   uint8_t *pageCount) {
  CLEAN_AND_CHECK()
...
parser_error_t
    _toStringEthereumAddress(const pd_EthereumAddress_t *v, char *outValue,
                             uint16_t outValueLen, uint8_t pageIdx,
                             uint8_t *pageCount){GEN_DEF_TOSTRING_ARRAY(20)}

parser_error_t _toStringStatementKind(const pd_StatementKind_t *v,
                                      char *outValue, uint16_t outValueLen,
                                      uint8_t pageIdx, uint8_t *pageCount) {
  CLEAN_AND_CHECK()
...
parser_error_t _toStringTupleAccountIdData(const pd_TupleAccountIdData_t *v,
                                           char *outValue, uint16_t outValueLen,
                                           uint8_t pageIdx,
                                           uint8_t *pageCount) {
  CLEAN_AND_CHECK()
...
parser_error_t _toStringRegistrarIndex(const pd_RegistrarIndex_t *v,
                                       char *outValue, uint16_t outValueLen,
                                       uint8_t pageIdx, uint8_t *pageCount) {
  return _toStringu32(&v->value, outValue, outValueLen, pageIdx, pageCount);
}

parser_error_t _toStringIdentityFields(const pd_IdentityFields_t *v,
                                       char *outValue, uint16_t outValueLen,
                                       uint8_t pageIdx, uint8_t *pageCount) {
  CLEAN_AND_CHECK()
...
parser_error_t _toStringJudgement(const pd_Judgement_t *v, char *outValue,
                                  uint16_t outValueLen, uint8_t pageIdx,
                                  uint8_t *pageCount) {
  CLEAN_AND_CHECK()
...
parser_error_t _toStringProxyType(const pd_ProxyType_t *v, char *outValue,
                                  uint16_t outValueLen, uint8_t pageIdx,
                                  uint8_t *pageCount) {
  CLEAN_AND_CHECK()
...
parser_error_t _toStringCallHashOf(const pd_CallHashOf_t *v, char *outValue,
                                   uint16_t outValueLen, uint8_t pageIdx,
                                   uint8_t *pageCount) {
  CLEAN_AND_CHECK()
...
parser_error_t _toStringTimepoint(const pd_Timepoint_t *v, char *outValue,
                                  uint16_t outValueLen, uint8_t pageIdx,
                                  uint8_t *pageCount) {
  CLEAN_AND_CHECK()
...
parser_error_t _toStringOpaqueCall(const pd_OpaqueCall_t *v, char *outValue,
                                   uint16_t outValueLen, uint8_t pageIdx,
                                   uint8_t *pageCount) {
  CLEAN_AND_CHECK()
...
parser_error_t
_toStringVecTupleAccountIdData(const pd_VecTupleAccountIdData_t *v,
                               char *outValue, uint16_t outValueLen,
                               uint8_t pageIdx, uint8_t *pageCount) {
...
parser_error_t _toStringOptionTupleBalanceOfBalanceOfBlockNumber(
    const pd_OptionTupleBalanceOfBalanceOfBlockNumber_t *v, char *outValue,
    uint16_t outValueLen, uint8_t pageIdx, uint8_t *pageCount) {
  CLEAN_AND_CHECK()
...
parser_error_t
_toStringOptionReferendumIndex(const pd_OptionReferendumIndex_t *v,
                               char *outValue, uint16_t outValueLen,
                               uint8_t pageIdx, uint8_t *pageCount) {
  CLEAN_AND_CHECK()
  ...
  *pageCount = 1;
  if (v->some > 0) {
    CHECK_ERROR(_toStringReferendumIndex(&v->contained, outValue, outValueLen,
                                         pageIdx, pageCount));
  } else {
    snprintf(outValue, outValueLen, "None");
  }
...
parser_error_t _toStringOptionAccountId(const pd_OptionAccountId_t *v,
                                        char *outValue, uint16_t outValueLen,
                                        uint8_t pageIdx, uint8_t *pageCount) {
  CLEAN_AND_CHECK()
  ...
  *pageCount = 1;
  if (v->some > 0) {
    CHECK_ERROR(_toStringAccountId(&v->contained, outValue, outValueLen,
                                   pageIdx, pageCount));
  } else {
    snprintf(outValue, outValueLen, "None");
  }
...
parser_error_t _toStringOptionStatementKind(const pd_OptionStatementKind_t *v,
                                            char *outValue,
                                            uint16_t outValueLen,
                                            uint8_t pageIdx,
                                            uint8_t *pageCount) {
  ...
  *pageCount = 1;
  if (v->some > 0) {
    CHECK_ERROR(_toStringStatementKind(&v->contained, outValue, outValueLen,
                                       pageIdx, pageCount));
  } else {
    snprintf(outValue, outValueLen, "None");
  }
...
parser_error_t _toStringOptionProxyType(const pd_OptionProxyType_t *v,
                                        char *outValue, uint16_t outValueLen,
                                        uint8_t pageIdx, uint8_t *pageCount) {
  CLEAN_AND_CHECK()
  ...
  *pageCount = 1;
  if (v->some > 0) {
    CHECK_ERROR(_toStringProxyType(&v->contained, outValue, outValueLen,
                                   pageIdx, pageCount));
  } else {
    snprintf(outValue, outValueLen, "None");
  }
...
parser_error_t _toStringOptionTimepoint(const pd_OptionTimepoint_t *v,
                                        char *outValue, uint16_t outValueLen,
                                        uint8_t pageIdx, uint8_t *pageCount) {
  CLEAN_AND_CHECK()
  ...
  *pageCount = 1;
  if (v->some > 0) {
    CHECK_ERROR(_toStringTimepoint(&v->contained, outValue, outValueLen,
                                   pageIdx, pageCount));
  } else {
    snprintf(outValue, outValueLen, "None");
  }
