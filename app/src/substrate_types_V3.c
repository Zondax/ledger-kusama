...
parser_error_t _readAccountVote_V3(parser_context_t* c, pd_AccountVote_V3_t* v)
{
    ...
    case 0:
        CHECK_INPUT();
        CHECK_ERROR(_readAccountVoteStandard_V3(c, &v->voteStandard))
        break;
    case 1:
        CHECK_INPUT();
        CHECK_ERROR(_readAccountVoteSplit_V3(c, &v->voteSplit))
        break;
    default:
        break;
    }
...
{
    CHECK_INPUT();
    return _readCompactPerBill_V3(c, &v->commission);
}
...
parser_error_t _readVecValidatorIndex_V3(parser_context_t* c, pd_VecValidatorIndex_V3_t* v){
    GEN_DEF_READVECTOR(ValidatorIndex_V3)
}
...
parser_error_t _readOptionReferendumIndex_V3(parser_context_t* c, pd_OptionReferendumIndex_V3_t* v)
{
    CHECK_ERROR(_readUInt8(c, &v->some))
    if (v->some > 0) {
        CHECK_ERROR(_readReferendumIndex_V3(c, &v->contained))
    }
    return parser_ok;
}
...
parser_error_t _readOptionStatementKind_V3(parser_context_t* c, pd_OptionStatementKind_V3_t* v)
{
    CHECK_ERROR(_readUInt8(c, &v->some))
    if (v->some > 0) {
        CHECK_ERROR(_readStatementKind_V3(c, &v->contained))
    }
    return parser_ok;
}
...
parser_error_t _readOptionTupleBalanceOfBalanceOfBlockNumber_V3(parser_context_t* c, pd_OptionTupleBalanceOfBalanceOfBlockNumber_V3_t* v)
{
    CHECK_ERROR(_readUInt8(c, &v->some))
    if (v->some > 0) {
        CHECK_ERROR(_readTupleBalanceOfBalanceOfBlockNumber_V3(c, &v->contained))
    }
    return parser_ok;
}
    ...
    uint8_t* pageCount)
{
    ...
    *pageCount = 0;
    ...
    if (pageIdx < pages[1]) {
        CHECK_ERROR(_toStringVote_V3(&v->vote, outValue, outValueLen, pageIdx, &pages[1]));
        return parser_ok;
    }
    pageIdx -= pages[1];
    ...
    uint8_t* pageCount)
{
    ...
    switch (v->value) {
    case 0:
        _toStringAccountVoteStandard_V3(&v->voteStandard, outValue, outValueLen, pageIdx, pageCount);
        break;
    case 1:
        _toStringAccountVoteSplit_V3(&v->voteSplit, outValue, outValueLen, pageIdx, pageCount);
        break;
    default:
    ...
    uint8_t* pageCount)
{
    ...
    outValue[offset++] = ' ';
    CHECK_PARSER_ERR(_toStringConviction_V3(&v->conviction,
        outValue + offset,
        outValueLen - offset,
        pageIdx,
        pageCount));
...
parser_error_t _toStringVecValidatorIndex_V3(
    ...
    uint8_t* pageCount)
{
    GEN_DEF_TOSTRING_VECTOR(ValidatorIndex_V3);
}
    ...
    uint8_t* pageCount)
{
    ...
    *pageCount = 1;
    if (v->some > 0) {
        CHECK_ERROR(_toStringReferendumIndex_V3(
            &v->contained,
            outValue, outValueLen,
            pageIdx, pageCount));
    } else {
    ...
    uint8_t* pageCount)
{
    ...
    *pageCount = 1;
    if (v->some > 0) {
        CHECK_ERROR(_toStringStatementKind_V3(
            &v->contained,
            outValue, outValueLen,
            pageIdx, pageCount));
    } else {
    ...
    uint8_t* pageCount)
{
    ...
    *pageCount = 1;
    if (v->some > 0) {
        CHECK_ERROR(_toStringTupleBalanceOfBalanceOfBlockNumber_V3(
            &v->contained,
            outValue, outValueLen,
            pageIdx, pageCount));
    } else {
...
parser_error_t _readProposal_V3(parser_context_t* c, pd_Proposal_V3_t* v)
{
    ...
    CHECK_ERROR(_readMethodBasic_V3(c, v->callIndex.moduleIdx, v->callIndex.idx, &v->method))
