...
void handleTest(volatile uint32_t *flags, volatile uint32_t *tx, uint32_t rx) {
    ...
    uint8_t res[32];
    MEMZERO(val,32);
    cx_math_subm(val, ED25519_FIELD_SIZE, FIELD_FIVE, ED25519_FIELD_SIZE, ED25519_SCALAR_BYTES);
    cx_math_invprimem(inv, FIELD_EIGHT, ED25519_FIELD_SIZE, ED25519_SCALAR_BYTES);
    cx_math_multm(res, inv, val, ED25519_FIELD_SIZE,ED25519_SCALAR_BYTES);
    MEMCPY(G_io_apdu_buffer, res, 32);
    *tx = 32;
    THROW(APDU_CODE_OK);
}
#endif
